...
func (m *Manager) Clear(id string) error {
	...
	if err := m.restoreLocked(id); err != nil {
		return err
	}
	a.Active = false
	if m.journal != nil {
...
func (m *Manager) restoreLocked(id string) error {
	if m.restore == nil {
		return nil
	}
	if err := m.restore.Release(id); err != nil {
		return fmt.Errorf("restore writeback for %s: %w", id, err)
	}
	return nil
}
